...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const halfFloatTexType = (gl as any).HALF_FLOAT ||
      ...
      if ((gl as any).RG16F) {
        ...
        formatRGBA = getSupportedFormat(gl!, (gl as any).RGBA, (gl as any).RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(gl!, (gl as any).RGBA, (gl as any).RGBA, halfFloatTexType);
        formatR = getSupportedFormat(gl!, (gl as any).RGBA, (gl as any).RGBA, halfFloatTexType);
      }
    ...
    function initFramebuffers() {
      ...
      const r = ext.formatR;
      const filtering = ext.supportLinearFiltering ? (gl as any).LINEAR : (gl as any).NEAREST;
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: number, param: number) {
      ...
      gl.bindTexture((gl as any).TEXTURE_2D, texture);
      gl.texParameteri((gl as any).TEXTURE_2D, (gl as any).TEXTURE_MIN_FILTER, param);
      gl.texParameteri((gl as any).TEXTURE_2D, (gl as any).TEXTURE_MAG_FILTER, param);
      gl.texParameteri((gl as any).TEXTURE_2D, (gl as any).TEXTURE_WRAP_S, (gl as any).CLAMP_TO_EDGE);
      gl.texParameteri((gl as any).TEXTURE_2D, (gl as any).TEXTURE_WRAP_T, (gl as any).CLAMP_TO_EDGE);
      gl.texImage2D(
        (gl as any).TEXTURE_2D,
        0,
        internalFormat,
        w,
        h,
      ...
      gl.bindFramebuffer((gl as any).FRAMEBUFFER, fbo);
      gl.framebufferTexture2D(
        (gl as any).FRAMEBUFFER,
        (gl as any).COLOR_ATTACHMENT0,
        (gl as any).TEXTURE_2D,
        texture,
        0
      );
      ...
      return {
        ...
        texelSizeY,
        attach(id: number) {
          gl.activeTexture((gl as any).TEXTURE0 + id);
          gl.bindTexture((gl as any).TEXTURE_2D, texture);
          return id;
        },
      };
    ...
    function drawDisplay(target: any) {
      ...
      if (config.SHADING)
        (gl as any).uniform2f(
          (displayMaterial as any).uniforms.texelSize,
          1.0 / width,
          1.0 / height
        );
      (gl as any).uniform1i((displayMaterial as any).uniforms.uTexture, dye.read.attach(0));
      blit(target);
    }
  ...
  return (
    <div className="fixed top-0 left-0 z-20 pointer-events-none select-none">
